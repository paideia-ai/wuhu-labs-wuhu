...
export class MockSession implements Session {
  ...
  private async _runGeneration(): Promise<void> {
    ...
    // Decide how many tool-call turns (1–4)
    const numTurns = 1 + Math.floor(Math.random() * 4)
  ...
  private async _consumeFollowUpMessages(): Promise<void> {
    ...
    // The agent block was already closed and agent-end was already added
    // by _runGeneration before calling this method. Just start a fresh
    // generation for the follow-up.
    this._working = false
